...
class _SendEmailScreenState extends ConsumerState<SendEmailScreen> {
  ...
  void handleSubmit() async {
    ...
    final resetPasswordState = ref.read(resetPasswordProvider);
    setState(() => _isLoading = false);

    if (resetPasswordState.hasError) {
      SnackbarWebWidget.error(
        context,
        resetPasswordState.error.toString(),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Si estamos en web (no mobile), centramos el formulario y limitamos el ancho.
    if (kIsWeb) {
      form = Center(
        child: Container(
          constraints: const BoxConstraints(
            maxWidth: 600, // o ajusta el valor que veas apropiado
            minWidth: 320,
          ),
