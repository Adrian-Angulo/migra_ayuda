...
class TableReviews extends ConsumerStatefulWidget {
  const TableReviews({super.key});

  @override
  ConsumerState<TableReviews> createState() => _TableReviewsState();
}

class _TableReviewsState extends ConsumerState<TableReviews> {
...
class ReviewDataSource extends DataTableSource {
  List<ReviewEntity> listRiview = [];
  List<ReviewEntity> listFilterRiview = [];
  final Function(ReviewEntity)? onRowSelected;
...
/*   void aplicarFiltros(String busqueda, String? servicioSeleccionado) {
    listFilterRiview = listRiview.where((enty) {
      final coincideTexto =
  ...
  void actualizarDatos(List<ReviewEntity> reviewsNew) {
    listRiview = reviewsNew;
    listFilterRiview = listRiview;
    notifyListeners();
  }
  ...
  DataRow? getRow(int index) {
    if (index >= listFilterRiview.length) return null;
    final review = listFilterRiview[index];
  ...
  @override
  int get rowCount => listFilterRiview.length;
